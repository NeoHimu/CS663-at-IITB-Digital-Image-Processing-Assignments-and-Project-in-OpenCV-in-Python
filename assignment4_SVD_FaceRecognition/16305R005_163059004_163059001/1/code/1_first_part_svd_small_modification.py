...
'''
for k in ks:
	for zp in mean_deducted_test:
		alpha_test = V[:,0:k].T.dot(zp)
		min_temp = 1000000000
		for idx in range(0,len(alpha[0])): #len(alpha[0]) : length of first row => total number of columns
			min_temp = min(min_temp, sum( (alpha.T[idx][0:k] - alpha_test)**2) )
		
		closest_alphas.append(min_temp)

#This needs to be plotted
closest_alphas = np.reshape(closest_alphas, (len(ks),len(mean_deducted_test)))
print(closest_alphas.shape)
'''
